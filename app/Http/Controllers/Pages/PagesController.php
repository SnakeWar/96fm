...

{
...

    {
        ...
        //retorna notícias destaques fixadas
        $fixed =  $this->post
            ...
            ->limit(2)
            ->get();
        //retorna notícias fixadas abaixo do primeiro destaque
        $fixed_secondary =  $this->post

            ->where('fixed_secondary', 1)
        ...
        if($fixed->count()==2 && $fixed_secondary->count()==1){
                //se há 2 notícias destaques fixadas
                $featureds =  $this->post
                ...
                ->whereNotIn('id', [$fixed[0]->id,$fixed[1]->id,$fixed_secondary[0]->id])
                ...
                ->get();
                //dd($featureds);
        }elseif($fixed->count()==2 && $fixed_secondary->count()==2){
            //se há 2 notícias destaques fixadas e 2 fixadas abaixdo do destaque principal
            $featureds =  $this->post
            ...
            ->whereNotIn('id', [$fixed[0]->id,$fixed[1]->id,$fixed_secondary[0]->id,$fixed_secondary[1]->id])
        ...
        }elseif(isset($fixed) && $fixed->count()>0 && $fixed_secondary->count()>0){
            //se há notícias destaques fixadas E notícias fixadas abaixo do destaque principal
            $featureds =  $this->post
            ...
            ->whereNotIn('id', [$fixed[0]->id,$fixed_secondary[0]->id])
            ...
            ->whereNotIn('id', [$fixed[0]->id,$fixed[1]->id])
            ...
            ->get();
            //dd($featureds);
        }elseif(isset($fixed) && $fixed->count()>0){
            //se há notícias destaques fixadas
            $featureds =  $this->post
            ...
            //se NÂO há notícias destaques fixadas
            $featureds =  $this->post
        ...
        $banner_top_youtube = $this->banner

            ->whereStatus(1)

            ->where('type', 'top_youtube')
        ...
        return view('pages.index2',
            ...
            [
                ...
                'lives' => $lives,
                'banner_top_youtube' => $banner_top_youtube
    ...
    public function news(Request $request,$blogId = null)
    ...
    public function enviar_contato(ContatoFormRequest $request)
    {
